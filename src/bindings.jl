...
@cenum xkblas_precision_t::UInt32 begin
    S = 0
    C = 1
    D = 2
    Z = 3
    XKBLAS_PRECISION_MAX = 4
end

@cenum xkblas_index_t::UInt32 begin
    I32 = 0
    I64 = 1
    XKBLAS_INDEX_MAX = 2
end

@cenum xkblas_kernel_t::UInt32 begin
    AXPBY = 0
    AXPY = 1
    DIVCOPY = 2
    DOT = 3
    ...
    POTRF = 18
    SPMV = 19
    XKBLAS_KERNEL_MAX = 20
end

function kernel_name(k)
    @ccall libxkblas.xkblas_kernel_name(k::xkblas_kernel_t)::Ptr{Cint}
end

function kernel_auto_tile(kernel, args, bs)
    @ccall libxkblas.xkblas_kernel_auto_tile(kernel::xkblas_kernel_t, args::Ptr{Cint}, bs::Ptr{Csize_t})::Cvoid
end
...
function cblas2clblast_op(trans)
    @ccall libxkblas.cblas2clblast_op(trans::Cint)::Cint
end

function cblas2clblast_side(side)
    @ccall libxkblas.cblas2clblast_side(side::Cint)::Cint
end

function cblas2clblast_uplo(uplo)
    @ccall libxkblas.cblas2clblast_uplo(uplo::Cint)::Cint
end

function cblas2clblast_diag(diag)
    @ccall libxkblas.cblas2clblast_diag(diag::Cint)::Cint
end

struct xkblas_conf_kernel_t
    tile::Csize_t
end

struct xkblas_conf_s
    kernels::NTuple{20, xkblas_conf_kernel_t}
end

const xkblas_conf_t = xkblas_conf_s

function init_conf(conf)
    @ccall libxkblas.xkblas_init_conf(conf::Ptr{xkblas_conf_t})::Cvoid
end

function cblas2cublas_op(trans)
    @ccall libxkblas.cblas2cublas_op(trans::Cint)::Cint
end

function cblas2cublas_side(side)
    @ccall libxkblas.cblas2cublas_side(side::Cint)::Cint
end

function cblas2cublas_uplo(uplo)
    @ccall libxkblas.cblas2cublas_uplo(uplo::Cint)::Cint
end

function cblas2cublas_diag(diag)
    @ccall libxkblas.cblas2cublas_diag(diag::Cint)::Cint
end

function cblas2cusparse_op(trans)
    @ccall libxkblas.cblas2cusparse_op(trans::Cint)::Cint
end
...
function cblas2hipblas_op(trans)
    @ccall libxkblas.cblas2hipblas_op(trans::Cint)::Cint
end

function cblas2hipblas_side(side)
    @ccall libxkblas.cblas2hipblas_side(side::Cint)::Cint
end

function cblas2hipblas_uplo(uplo)
    @ccall libxkblas.cblas2hipblas_uplo(uplo::Cint)::Cint
end

function cblas2hipblas_diag(diag)
    @ccall libxkblas.cblas2hipblas_diag(diag::Cint)::Cint
end
...
# Skipping MacroDefinition: DEFINE_ENUM ( name ) name ,

# Skipping MacroDefinition: CASE_NAME ( name ) case name : return # name ;
...
const XKBLAS_SUPPORT_CBLAS = 0

const XKBLAS_SUPPORT_CUDA = 1

const XKBLAS_SUPPORT_CUBLAS = 1

const XKBLAS_SUPPORT_HIP = 0

const XKBLAS_SUPPORT_HIPBLAS = 0

const XKBLAS_SUPPORT_CLBLAST = 0

const XKBLAS_SUPPORT_ZE = 0

const XKBLAS_SUPPORT_SYCL = 0
