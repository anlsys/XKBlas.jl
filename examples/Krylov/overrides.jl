...
function xkdot(n::Integer, x::Vector{T}, y::Vector{T}) where T <: FloatOrComplex
    r = Ref{T}(0)
    # TODO: this is wrong for complex. Need to call dotc here for complex numbers
...
Krylov.kmul!(y::XKVector{T}, P::XKSparseMatrixCSR, x::XKVector{T}) where T <: FloatOrComplex = XK.BLAS.spmv_sync(T(1.0), XK.BLAS.NO_TRANS, size(P.data, 1), size(P.data, 2), length(P.data.nzval), XK.BLAS.SPARSE_CSR, P.data.rowptr, P.data.colval, P.data.nzval, x.data, T(0.0), y.data)
Krylov.kmul!(y::XKVector{T}, P::XKMatrix,          x::XKVector{T}) where T <: FloatOrComplex = XK.BLAS.gemv_sync(XK.BLAS.NO_TRANS, size(P.data, 1), size(P.data, 2), T(1.0), p.data, stride(p.data, 2), x.data, 1, T(0.0), y.data, 1)
